...
# set all NAs to 0, since 0/0 does not impact binomial model and NAs
# happen in the same place for both alleles
c57.ss2[which(is.na(c57.ss2))] <- 0
cast.ss2[which(is.na(cast.ss2))] <- 0
```
...
# TRY SETTING ALL NAs to 0
c57.ss3[which(is.na(c57.ss3))] <- 0
cast.ss3[which(is.na(cast.ss3))] <- 0
...
print(paste('Average coverage/cell for SS3:', mean(colMeans(c57.ss3+cast.ss3))))
print(paste('Average coverage/cell for SS2 w/ UMIs:', mean(colMeans(c57.ss2+cast.ss2))))
```
...
Compare this to using `apeglm`:

```{r}
ase.cts <- c57.ss2
cts <- c57.ss2+cast.ss2
...
# theta.hat <- bbEstDisp(success=ase.cts, size=cts,x=X, beta=fit.mle$map,minDisp=1, maxDisp=500)
# param <- cbind(theta.hat, cts)
# fit.mle.ss2 <- apeglm(Y=ase.cts,x=X,log.lik=NULL,param=param,no.shrink=TRUE,log.link=FALSE,method="betabinCR")
...
param <- cbind(theta.hat, cts)
fit.map.ss2 <- apeglm(Y=ase.cts, x=X, log.lik=NULL, param=param, coef=1, mle=mle, log.link=FALSE, method = "betabinCR")
```

```{r}
ase.cts <- c57.ss3
cts <- c57.ss3+cast.ss3
X <- matrix(rep(1,ncol(cts)))
# theta.hat.0 <- 100 # rough estimate of dispersion
# param <- cbind(theta.hat.0, cts)
# fit.mle <- apeglm(Y=ase.cts,x=X,log.lik=NULL,param=param,no.shrink=TRUE,log.link=FALSE,method="betabinC")
...
# theta.hat <- bbEstDisp(success=ase.cts, size=cts,x=X, beta=fit.mle$map,minDisp=1, maxDisp=500)
# param <- cbind(theta.hat, cts)
# fit.mle.ss3 <- apeglm(Y=ase.cts,x=X,log.lik=NULL,param=param,no.shrink=TRUE,log.link=FALSE,method="betabinCR")
...
mle <- cbind(fit.mle$map, fit.mle$sd)
param <- cbind(theta.hat, cts)
fit.map.ss3 <- apeglm(Y=ase.cts, x=X, log.lik=NULL, param=param, coef=1, mle=mle, log.link=FALSE, method = "betabinCR")
```

```{r}
apeglm_dd <- data.frame(gene = rownames(fit.map.ss3$map), map.ss3 = fit.map.ss3$map) %>%
  left_join(data.frame(gene = rownames(fit.map.ss2$map), map.ss2 = fit.map.ss2$map), by='gene') %>%
  filter(!is.na(map.ss2), !is.na(map.ss3))

p1 <- apeglm_dd %>%
  ggplot(aes(x = expit(map.ss2), y = expit(map.ss3))) +
  ...
  xlab(TeX(r'($\hat{p}$, Smart-seq2 with UMIs)')) +
  ylab(TeX(r'($\hat{p}$, Smart-seq3)')) +
  ggtitle(paste0('apeglm, r2=',round(cor(expit(apeglm_dd$map.ss2), 
                                         expit(apeglm_dd$map.ss3)),2)))


p2 <- both %>%
  filter(gene %in% apeglm_dd$gene) %>%
  ggplot(aes(x = p.2, y = p.3)) +
  geom_point(alpha=0.1) +
  geom_abline(slope=1, intercept=0, color='red', lty='dashed') +
  theme_bw() +
  ...
  xlab(TeX(r'($\hat{p}$, Smart-seq2 with UMIs)')) +
  ylab(TeX(r'($\hat{p}$, Smart-seq3)')) +
  ggtitle(paste0('beta-binomial, r2=',round(cor(both$p.2,both$p.3),2)))
ggsave('apeglm-vs-me.png', plot = arrangeGrob(p1, p2, nrow=1), height=4, width=8)
