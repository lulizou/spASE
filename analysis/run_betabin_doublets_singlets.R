...
singlet_beads <- cell_types |>
  filter(spot_class == 'singlet') |>
  pull(bead)
doublet_beads <- cell_types |>
  filter(grepl('doublet', spot_class)) |>
  pull(bead)

maternal_counts<- counts1 |>
  bind_rows(counts2) |>
  filter(bead %in% singlet_beads) |>
  arrange(bead, gene) |>
...
saveRDS(myfit, 'betabin_results_overall_bias_cere_4_singlets.rds')


maternal_counts<- counts1 |>
  bind_rows(counts2) |>
  filter(bead %in% doublet_beads) |>
  arrange(bead, gene) |>
  mutate(bead = as.factor(bead), gene = as.factor(gene))
maternal_counts_matrix <- sparseMatrix(i=maternal_counts$gene, j=maternal_counts$bead, x=maternal_counts$CAST)
rownames(maternal_counts_matrix) <- levels(maternal_counts$gene)
...
myfit <- scase(maternal_counts_matrix, paternal_counts_matrix, min.cells = 2^7, cores=8, verbose=T)
saveRDS(myfit, 'betabin_results_overall_bias_cere_4_doublets.rds')
...
saveRDS(myfit_celltype, 'betabin_results_celltype_bias_cere_4.rds')
