...
```{r}
cere3 <- readRDS('cside_spase_combined_cere4_visium_df_5.rds')
cere4 <- readRDS('cside_spase_combined_cere_4_df_5.rds')
hippo1 <- readRDS('cside_spase_combined_hippo_1_df_5.rds')
hippo2 <- readRDS('cside_spase_combined_hippo_2_df_5.rds')
hippo3 <- readRDS('cside_spase_combined_hippo_3_df_5.rds')
```
...
ggsave('figures/02.5_cside_estimates_hippo.pdf', height=6, width=5)
```

```{r}
cere4@de_results$gene_fits$mean_val |>
  melt() |>
  mutate(sample = 'Cerebellum 3 (SS)') |>
  bind_rows(cere3@de_results$gene_fits$mean_val |>
              melt() |>
  ...
  xlim(c(-10,10)) +
  xlab('Estimated coefficient (log scale)') 
ggsave('figures/02.5_cside_estimates_cere.pdf', height=6, width=3)
```
...
ggsave('figures/02.5_cside_estimate_variances.pdf', height=3, width=5)
```
...
cbind(coords, data.frame(yij = y_ij, nij = N_ij)) |>
  ...
  theme_void() +
  theme(legend.position='none')
ggsave('figures/02.5_Chd7_sim_counts.pdf', height=1, width=2)
```
...
```{r}
visium_params <- fread('simulations_cere3_params.tsv')
visium_results <- fread('simulation_results_cere3.csv', col.names=c('idx','total_N', 'avg_N', 'median_N', 'MSE', 'avg_var', 'coverage', 'cor_p'))
visium_params$idx <- 1:nrow(visium_params)
visium_results <- left_join(visium_params, visium_results, by = 'idx') |>
  mutate(platform = 'Visium') 

ss_params <- fread('simulations_cere4_params.tsv')
ss_results <- fread('simulation_results_cere4.csv', col.names=c('idx','total_N', 'avg_N', 'median_N', 'MSE', 'avg_var', 'coverage', 'cor_p'))
...
```{r}
aa <- all_results |>
  mutate(bin_N = cut(total_N, breaks=c(0,1000,5000,10000,250000))) |>
  filter(!is.na(bin_N))
...
aa |>
  filter(cell_type != 'Bergmann', cell_type != 'Fibroblast') |>
  ...
  ylab(TeX(r"($R^2$)")) +
  xlab('Total UMI')
ggsave('figures/02.5_simulations_r2.pdf', height=3, width=7)
```
...
aa |>
  ...
  xlab('Total UMI')
ggsave('figures/02.5_simulations_rmse.pdf', height=3, width=7)
```
...
all_results |>
  ...
  ylab('Density')
ggsave('figures/02.5_simulations_coef_var.pdf', height=3, width=4)
```
...
aa |>
  ...
  ylab(TeX(r"($R^2$)")) +
  xlab('Overdispersion')
ggsave('figures/02.5_simulations_overdispersion.pdf', height=3, width=3)
```


```{r}
all_results |>
  mutate(cov = coverage/5) |>
  mutate(bin_N = cut_number(total_N, 4)) |>
  group_by(cell_type, platform, bin_N) |>
