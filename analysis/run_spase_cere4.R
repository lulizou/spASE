...
myRCTD <- readRDS('cere_slideseq_mouse_4_1.rds')
...
saveRDS(myRCTD, file.path('cside_spase_cere_4_1.rds'))

myRCTD <- readRDS('cere_slideseq_mouse_4_2.rds')
myRCTD@config$max_cores <- 8
...
myRCTD <- run.CSIDE.nonparam(myRCTD, df = 5, cell_types = cell_types,
                             cell_type_threshold = 50, spase = T)
saveRDS(myRCTD, file.path('cside_spase_cere_4_2.rds'))
