...
```{r}
tx <- readDNAStringSet('/rafalab/lzou/slideseq/emase.pooled.targets.fa')
ensembl_ids <- read.csv('/rafalab/lzou/slideseq/ensembl_tx_gene.csv')
...
for (i in 1:nrow(df)) {
  ...
  ggsave(paste0('figures/06_',df[i,2],'_',df[i,1],'.pdf'),height=1, width=8)
}
```
...
saveRDS(tx_df, file = '06_129_CAST_df.rds')
```

```{r}
tx_df <- readRDS('06_129_CAST_df.rds')
```
...
cere3 <- readRDS('cside_spase_combined_cere_4_df_5.rds')
gene_counts <- rowSums(cere3@spatialRNA@maternalCounts)+rowSums(cere3@spatialRNA@paternalCounts)
...
ggsave('figures/06_129_CAST_nmismatch_vs_count.pdf', height=3, width=6)
```
