...
myRCTD <- readRDS('cside_spase_cere_3.rds')
params <- read.delim('simulations_cere3_params.tsv')
df <- 5
cell_types <- c('Granule', 'MLI2','Purkinje','Oligodendrocytes')
...
for (i in idx) {
  ...
  cat(paste(c(i,round(metrics,4)), collapse=','), file = 'simulation_results_cere3.csv', sep='\n', append=T)
}
