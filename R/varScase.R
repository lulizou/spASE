...
varScase <- function(matrix1, matrix2, scasefit, genes=NULL, save=NULL,
  ...
  df.hist$totalBin <- cut(df.hist$total, breaks=breaks, include.lowest=T)
  # color palette
  ...
  for (i in 1:length(genes)) {
    ...
    p <- p +
      ...
      scale_fill_manual(name = 'total UMI per cell', values = colors.subset) +
      theme_classic() +
  ...
  # finally, plot p and phi for all genes
  df.pdf$totalBin <- cut(df.pdf$totalUMI, breaks=c(0,100,250,500,1e3,5e3,1e4,
                                                   2e4,5e4,1e5,5e5,1e6), include.lowest=T)
  mycolors <- viridis(length(unique(df.pdf$totalBin)))
  pidx <- which(df.pdf$gene %in% genes)
  labelthese <- df.pdf[pidx,]
  pphi <- ggplot(df.pdf, aes(x = p, y = phi)) +
    geom_point(aes(color = totalBin), alpha=0.5) +
    scale_color_manual(name='total UMI', values=mycolors) +
    annotate('point', df.pdf$p[pidx], df.pdf$phi[pidx], color='black') +
    geom_label_repel(data = labelthese, aes(label=gene), fill='white',
                     size=3, label.padding=0.05, min.segment.length=0) +
    theme_classic() +
    theme(axis.title = element_text(size=10), axis.text = element_text(size=9),
          plot.title = element_text(size=12)) +
    xlab(TeX('estimated p maternal')) +
    ylab('estimated phi')
