...
#' @slot maternalCounts a sparse matrix of raw maternal counts for each gene (rowname) and each cell (colnames or barcodes)
#' @slot paternalCounts a sparse matrix of raw paternal counts for each gene (rowname) and each cell (colnames or barcodes)
...
setClass("SpatialRNA",
   slots = c(
     coords = "data.frame",
     counts = "dgCMatrix",
     maternalCounts = "dgCMatrix",
     paternalCounts = "dgCMatrix",
     nUMI = "numeric"
   ),
   prototype = list(
