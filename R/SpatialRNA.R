...
#' @slot maternalCounts Optional, for allele specific expression, a matrix (or dgCmatrix) of raw maternal counts for each gene (rowname) and each pixel (colnames or barcodes)
#' @slot paternalCounts Optional, for allele specific expression, a matrix (or dgCmatrix) of raw paternal counts for each gene (rowname) and each pixel (colnames or barcodes)
...
SpatialRNA <- function(coords, counts, nUMI = NULL, use_fake_coords = FALSE, require_int = TRUE,
                       maternalCounts = NULL, paternalCounts = NULL) {
  ...
  if(!is.null(maternalCounts)) {
    maternalCounts <- check_counts(maternalCounts, 'SpatialRNA', require_int = require_int)
    if(length(setdiff(barcodes, colnames(maternalCounts))) > 0)
      stop('SpatialRNA: missing barcodes from maternalCounts that are present in counts')
    if(any(rownames(maternalCounts) != rownames(counts)))
      stop('SpatialRNA: maternalCounts rownames (genes) do not match counts rownames')
    maternalCounts <- maternalCounts[,barcodes]
  }
  if(!is.null(paternalCounts)) {
    paternalCounts <- check_counts(paternalCounts, 'SpatialRNA', require_int = require_int)
    if(length(setdiff(barcodes, colnames(paternalCounts))) > 0)
      stop('SpatialRNA: missing barcodes from paternalCounts that are present in counts')
    if(any(rownames(paternalCounts) != rownames(counts)))
      stop('SpatialRNA: paternalCounts rownames (genes) do not match counts rownames')
    paternalCounts <- paternalCounts[,barcodes]
  }
  new("SpatialRNA", coords = coords[barcodes,], counts = counts[,barcodes], nUMI = nUMI[barcodes],
      maternalCounts = maternalCounts, paternalCounts = paternalCounts)
}
...
restrict_counts <- function(puck, gene_list, UMI_thresh = 1, UMI_max = 20000, counts_thresh = 1) {
  ...
  if(!is.null(puck@maternalCounts))
    puck@maternalCounts <- puck@maternalCounts[gene_list,keep_loc]
  if(!is.null(puck@paternalCounts))
    puck@paternalCounts <- puck@paternalCounts[gene_list,keep_loc]
...
restrict_puck <- function(puck, barcodes) {
  ...
  if(!is.null(puck@maternalCounts))
    puck@maternalCounts <- puck@maternalCounts[,barcodes]
  if(!is.null(puck@paternalCounts))
    puck@paternalCounts <- puck@paternalCounts[,barcodes]
  return(puck)
}
