...
calc_Q_k.spASE <- function(lambda, y, nUMI, r) {
  # lambda = p, we know phi.
  #r <- (1 - phi) / phi
  a = lambda * r
  b = (1 - lambda) * r
  choose(nUMI, y) * beta(y + a, nUMI - y + b) / beta(a, b)
}

#negative log likelihood
calc_log_l_vec.spASE <- function(lambda, Y, nUMI, r, return_vec = FALSE) {
  log_l_vec <- -log(calc_Q_k.spASE(lambda,Y, nUMI, r))
  if(return_vec)
    return(log_l_vec)
...
get_d1_d2.spASE <- function(Y, lambda, nUMI, r) {
  #r <- (1 - phi) / phi
  a = lambda * r; b = (1 - lambda) * r
  d1_vec <- r * (digamma(Y + a) - digamma(nUMI - Y + b) - digamma(a) + digamma(b))
  d2_vec <- (r^2) * (psigamma(Y + a, 1) + psigamma(nUMI - Y + b, 1) -
                       psigamma(a, 1) - psigamma(b, 1))
  return(list(d1_vec = d1_vec, d2_vec = d2_vec))
}

get_d1_d2 <- function(Y, lambda, spase = F, nUMI = NULL, r = NULL) {
  if(spase) {
    return(get_d1_d2.spASE(Y, lambda, nUMI, r))
  } else {
...
#negative log likelihood
calc_log_l_vec <- function(lambda, Y, spase = F, nUMI = NULL, r = NULL, return_vec = FALSE) {
  if(!spase)
    return(calc_log_l_vec_fast(lambda, Y))
  else
    return(calc_log_l_vec.spASE(lambda, Y, nUMI, r, return_vec = return_vec))
}
